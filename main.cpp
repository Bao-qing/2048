...
public:
    ...
    // 返回值：无
    static void transposition_matrix(int matrix[4][4], SDL_Scancode direction) {
        // 矩阵的不同转置，用于不同方向的移动
        int i, j;
...
public:
    ...
    */
    ...
    // 返回值：无
    void animation(int move[4][4], int all[4][4], SDL_Scancode direction, int score, int max_score, int scoreadd,
                   uint8_t loadAnimation) {
...
int main() {
    ...
    while (true) {
        ...
        if (SDL_PollEvent(&evt)) // 有事件，SDL_PollEvent用于取出事件
        {
            ...
            if (evt.type == SDL_MOUSEBUTTONDOWN) // 鼠标点击事件，只用于gameover时检测是否点击了tryagain
            {
                if (evt.button.x > renderer.rect_tryagain.x && evt.button.x < renderer.rect_tryagain.x + renderer.
                    rect_tryagain.w &&
                    evt.button.y > renderer.rect_tryagain.y && evt.button.y < renderer.rect_tryagain.y + renderer.
                    rect_tryagain.h &&
            ...
            } else if (evt.type == SDL_KEYDOWN) // 键盘按下事件
            {
                ...
                if (grid.moveFlag != 0) // 有移动，进行动画，否则不进行动画，以节约资源
                {
                    ...
                    renderer.animation(grid.cellsMove, grid.cells, evt.key.keysym.scancode, scoreManager.score,
                                       max_score,

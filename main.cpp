...
void animation(int move[4][4], int all[4][4], SDL_Renderer *pRenderer, SDL_Texture *tex /*背景*/, SDL_Scancode direction,
...
int main()
{
    ...
    int i, j;
    int new_block_pos1[2] = {0};
    int new_block_pos2[2] = {0};
    ...
    // 绘图
    draw(pRenderer, grid.cells, tex, font, 0, 0);
        ...
        if (SDL_PollEvent(&evt)) // 有事件，SDL_PollEvent用于取出事件
        {
            ...
            else if (evt.type == SDL_KEYDOWN) // 键盘按下事件
            {
                    ...
                    evt.key.keysym.scancode != SDL_SCANCODE_DOWN) // 按下的不是wasd 或小键盘，不响应
                {
                    ...
                    {
                        grid.cells[3][3] = 1024;
                        draw(pRenderer, grid.cells, tex, font, gameover, score);
                    }
                    continue;
                }
                int score_add = grid.renewBoard(evt.key.keysym.scancode); // 记录单步分数
                int move_flag = grid.moveFlag;
                if (move_flag != 0) // 有移动，进行动画，否则不进行动画，以节约资源
                {
                    ...
                    animation(grid.cellsMove, grid.cells, pRenderer, tex, evt.key.keysym.scancode, font, score, max_score,
                ...
                if (gameover == 1)
                {
                    draw(pRenderer, grid.cells, tex, font, gameover, score); // 先绘制正常界面，再延迟后绘制游戏结束界面
                    ...
                    draw(pRenderer, grid.cells, tex, font, gameover, score); // 绘图，以游戏结束形式绘图
                    continue;
                }
                else if (gameover == 2)
                {
                ...
                bubble_animation(pRenderer, grid.cellsUsed, grid.cellsMove, grid.cellsTemp, grid.cells, tex, font, score, max_score); // 膨胀动画
                draw(pRenderer, grid.cells, tex, font, gameover, score);                                    // 绘图，正常绘图
            }
        }
    }
